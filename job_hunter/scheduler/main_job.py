...
from job_hunter.email.sender import send_daily_email, send_system_email
...
def run_pipeline():
    try:
        ...
        send_system_email(
            subject="Alert: Job Scraper Pipeline Failed",
            body=f"The job scraper pipeline encountered an error during its hourly run:\n\n{str(e)}\n\nPlease check the server/Action logs for more details."
        )
        raise e
